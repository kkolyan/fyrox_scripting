...
fn main() {
    ...
    let target_dir = "book/src/scripting_api_cs";
    ...
    let summary_template = fs::read_to_string("book/src/SUMMARY.template.md").unwrap();
    let summary = summary_template.replace("%GENERATED_SECTION%", &summary_section);
    fs::write("book/src/SUMMARY.md", summary).unwrap();
}
