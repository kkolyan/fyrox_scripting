...
# fyroxed-c is built too to avoid rebuilds between edit and play (not sure it works that way, need to check)
RUSTFLAGS='-C prefer-dynamic=yes' cargo build -p fyrox-c -p fyroxed-c
cp target/debug/fyrox_c.dll cs/examples/Guards/bin/Debug/net8.0/
